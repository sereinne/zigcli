...
const parser = @import("value_parser");
// struct aliases
...
pub fn CLIApp(comptime CLITemplate: type, comptime Fallback: type, comptime config: Config(Fallback)) type {
    ...
    return struct {
        ...
        pub fn init(allocator: Allocator) Self {
            ...
            return Self{ .inner = undefined, .argsiter = ArgumentParserIterator.init(argiter) };
        }
...
const Command = struct {
    help: bool = false,
    version: bool = false,
    message: []const u8 = "cmd",
};

pub fn main() !void {
    var cli = CLIApp(Command, void, .{}).default();
    print("{}\n", .{cli.inner.help});
    print("{}\n", .{cli.inner.version});
    print("{s}\n", .{cli.inner.message});
    try cli.parse();
